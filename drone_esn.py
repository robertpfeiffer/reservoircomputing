...
class DroneESN(object):
    ...
    def compute(self, data):
        ...
        #time, yaw, pitch, roll, altitude, x, y, z, targetX, targetY, targetZ 
        if self.last_time is None:
            self.last_time = timestamp_to_date(float(data[0])-0.1)
        data[0] = compute_time_diff_in_ms(self.last_time, data[0])
        data[8:] = data[8:] - data[5:8] #relativer target-vector
...
if __name__ == '__main__':
    ...
    row_data = np.column_stack((np.asarray(flight_data.dataTimeDiffs), np.asarray(flight_data.dataYaw),
                                     np.asarray(flight_data.dataPitch), np.asarray(flight_data.dataRoll), np.asarray(flight_data.dataAltitude),
