...
#        SOCK_COMMANDER.setblocking(0)
    ...
    #            if cycles<int(elapsed*freq):
                if navdata and tracker_data and 0 in navdata:
                    yStart=tracker_data.find('y')
                    zStart=tracker_data.find('z')
                    zEnd=tracker_data.find('#')
                    x=float(tracker_data[1:yStart])
                    y=float(tracker_data[yStart+1:zStart])
                    z=float(tracker_data[zStart+1:zEnd])
                    
                    if navdata[0]['battery']<>oldBattery:
                        print 'Battery1: ' + str(navdata[0]['battery'])
                        oldBattery=navdata[0]['battery']
                    measurement=[navdata[0][a] for a in dims]+[x]+[y]+[z]+ [command_data] + [time.time()]
                    print measurement
